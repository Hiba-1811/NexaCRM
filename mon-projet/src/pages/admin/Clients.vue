  ...
  <div class="clients-page">
    ...
    <div class="stats-bar">
      <div class="stat-card">
        ...
        <span class="stat-label">Nombre total de clients</span>
      </div>
      <div class="stat-card">
        <span class="stat-number green">{{ activeClients }}</span>
        <span class="stat-label">Clients Actifs</span>
      </div>
      <div class="stat-card">
        <span class="stat-number red">{{ inactiveClients }}</span>
    ...
    <div class="page-header">
      <div class="header-actions">
        <button @click="showAddModal = true" class="btn-primary">+ Ajouter un client</button>
        <button @click="$router.push('/clients/import')" class="btn-import">📥 Importer des clients</button>
...
const activeClients = computed(() => clients.value.filter(c => c.client_status === 'active').length)
const inactiveClients = computed(() => clients.value.filter(c => c.client_status === 'inactive').length)
...
.stat-card { background: white; border-radius: 10px; padding: 16px 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); display: flex; flex-direction: column; gap: 4px; min-width: 160px; border: 1px solid #f0f0f0; }
...
.btn-import { background: linear-gradient(135deg, #11998e, #38ef7d); color: white; padding: 10px 20px; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; transition: transform 0.2s; }
